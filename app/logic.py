...
def _call_ai_batch_analysis(
    entries: list[Dict[str, Any]],
    cfg: Dict,
) -> str:
    """
    调用 AI 对多条告警进行批量研判（例如同源 IP 或同时间窗的一组日志）。

    Args:
        entries: 历史记录列表，每项为 HistoryPage 保存的 entry（包含 parsed_data/ti_result/raw_text 等）。
        cfg: 配置字典

    Returns:
        str: AI 批量分析结果（整体攻击意图/阶段/目标/优先级建议等）
    """
    ...
    # 构造多告警批量研判的提示词
    prompt = _build_ai_batch_prompt(entries, cfg)
...
def _build_ai_batch_prompt(
    ...
    相比单条告警，更关注整体攻击意图、阶段划分、关键资产与优先处置建议。
    """
    ai_cfg = cfg.get("ai", {}) or {}
    # 在原有 objective 基础上，默认追加一段“整体波次”研判目标
    base_objective = ai_cfg.get(
        "objective",
        "对该告警进行专业安全威胁研判，给出威胁等级、是否需要封禁源IP/目的IP以及主要原因与风险分析。",
    ).strip()
    objective = (
        "现在不是单条告警，而是多条可能属于同一攻击波次或同一演练场景的告警。"
        "请站在整体攻击视角进行研判，"
        "识别攻击阶段、主要攻击意图、目标资产与业务影响，"
        "并给出本轮攻击的总体风险等级和优先处置建议。\n"
        f"单条告警的默认分析目标如下，仅供参考，可在整体研判中综合考虑：{base_objective}"
    )
    ...
    lines.append("【C: Context】")
    lines.append(
        "下面是一组解析好的安全告警记录，它们可能属于同一攻击者、同一攻击波次或同一演练场景。"
        "每条告警都包含解析字段和（如有）威胁情报摘要。请基于整体上下文进行研判。"
    )
    lines.append("")
    ...
    # 限制数量，避免上下文过长（例如最多 20 条）
    max_entries = 20
    sliced_entries = entries[:max_entries]
    if len(entries) > max_entries:
        lines.append(
            f"（注意：当前一共选中了 {len(entries)} 条告警，为控制上下文长度，仅展示前 {max_entries} 条进行研判。）"
        )
        lines.append("")

    for idx, entry in enumerate(sliced_entries, start=1):
        pd = entry.get("parsed_data") or {}
        ti = entry.get("ti_result") or {}
        ts = entry.get("detected_at") or ""

        lines.append(f"--- 告警 #{idx} ---")
        if ts:
            lines.append(f"研判发现时间: {ts}")
        ...
        # 其他解析字段
        other_items = [(k, v) for k, v in pd.items() if k not in required_keys and v]
        if other_items:
            lines.append("其他解析字段:")
        ...
        # 威胁情报摘要（如存在）
        if ti:
            lines.append("威胁情报摘要（ThreatBook）:")

            def fmt_ti(prefix: str, ti_one: Dict) -> None:
                if not ti_one:
                    return
                ip = ti_one.get("ip")
                is_mal = ti_one.get("is_malicious")
                labels = ti_one.get("labels") or []
                loc = ti_one.get("location") or {}
                loc_parts = [loc.get("country"), loc.get("province"), loc.get("city")]
                loc_text = " / ".join([p for p in loc_parts if p])
                if ip:
                    lines.append(f"- {prefix} IP: {ip}；是否恶意: {'是' if is_mal else '否'}")
            ...
            if ti.get("src_ip_ti"):
                fmt_ti("源", ti["src_ip_ti"])
            if ti.get("dst_ip_ti"):
                fmt_ti("目的", ti["dst_ip_ti"])
    ...
    lines.append("【S: Style】")
    lines.append(
        "采用专业安全威胁分析人员的风格，结构清晰、分点说明，"
        "在描述整体攻击时注意区分不同攻击阶段（例如：侦察、利用、持久化、横向移动、数据访问等）。"
    )
    ...
    if audience == "customer":
        lines.append(
            "受众为业务方/客户：请用相对通俗的语言解释本轮攻击对业务的影响，"
            "避免过多技术细节，更关注“发生了什么 / 风险有多大 / 应该怎么做”。"
        )
    elif audience == "beginner":
        lines.append(
            "受众为安全初学者：尽量避免晦涩缩写，如必须使用（如 C2、Botnet 等），"
            "请在首次出现时用括号简要解释。"
        )
    else:
        lines.append(
            "受众为专业安全人员：可以使用 ATT&CK、C2、漏洞利用链等专业术语，"
            "重点说明研判依据和攻击阶段划分。"
        )
    ...
    if response_mode == "brief":
        lines.append(
            "请用不超过 5 行给出整体结论：包括本轮攻击的总体风险等级、是否建议封禁相关 IP/账号、"
            "以及 2-3 个关键依据（例如：是否命中高危 TI、是否成功触达关键资产等）。"
        )
    elif response_mode == "report":
        lines.append(
            "请以 Markdown 二级标题分段输出完整报告，建议结构："
            "“## 摘要”、“## 攻击阶段与时间线”、“## 受影响资产”、“## 处置建议”。"
        )
    else:
        # structured
        lines.append(
            "请使用如下中文结构化格式输出（字段名保持不变，每行一项）：\n"
            "总体威胁等级：<高/中/低/信息>\n"
            "主要攻击阶段：<简要按阶段概括，如“扫描/利用/横向移动”等>\n"
            "主要攻击意图：<例如“探测弱口令”、“利用 Web 漏洞获取权限”等>\n"
            "关键受影响资产：<列出最需要关注的 IP/系统/业务>\n"
            "优先处置建议：<列出 2-4 条可执行的处置建议>\n"
            "补充说明：<如有需要，可补充对演练效果、规则策略改进的建议>"
        )
