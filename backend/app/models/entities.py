...
class TimestampMixin:
    ...
    updated_at: Mapped[datetime] = mapped_column(
        DateTime, default=now, onupdate=now, nullable=False, index=True
    )
...
class Device(Base, TimestampMixin):
    ...
    name: Mapped[str] = mapped_column(String(160), nullable=False)
    device_role: Mapped[str] = mapped_column(String(40), default="monitor", nullable=False)
    browser_assistant_enabled: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
    browser_url_patterns: Mapped[list] = mapped_column(JSON, default=list, nullable=False)
...
class Alert(Base, TimestampMixin):
    ...
    __table_args__ = (
        ...
        Index("ix_alerts_workspace_group", "workspace_id", "current_group"),
        Index("ix_alerts_workspace_assignee", "workspace_id", "assignee_id"),
        Index("ix_alerts_workspace_created", "workspace_id", "created_at"),
        Index("ix_alerts_workspace_status_group", "workspace_id", "status", "current_group"),
    )
    ...
    reported_by_name: Mapped[str] = mapped_column(String(120), default="", nullable=False)
    analysis_result: Mapped[str] = mapped_column(String(120), default="", nullable=False)
    is_emergency: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    block_device_ids: Mapped[list] = mapped_column(JSON, default=list, nullable=False)
    block_at: Mapped[datetime | None] = mapped_column(DateTime, nullable=True)
    response_note: Mapped[str] = mapped_column(Text, default="", nullable=False)
    response_owner_id: Mapped[int | None] = mapped_column(ForeignKey("users.id"), nullable=True)
...
class AiMemory(Base, TimestampMixin):
    ...
    __table_args__ = (UniqueConstraint("workspace_id", "conversation_id", "agent", name="uq_ai_memory_conversation_agent"),)

    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    workspace_id: Mapped[int] = mapped_column(ForeignKey("workspaces.id"), nullable=False, index=True)
    conversation_id: Mapped[int] = mapped_column(ForeignKey("ai_conversations.id", ondelete="CASCADE"), nullable=False, index=True)
    agent: Mapped[str] = mapped_column(String(60), default="threat", nullable=False, index=True)
...
class AiRun(Base, TimestampMixin):
    ...
    workspace_id: Mapped[int] = mapped_column(ForeignKey("workspaces.id"), nullable=False, index=True)
    actor_id: Mapped[int | None] = mapped_column(ForeignKey("users.id"), nullable=True, index=True)
    source: Mapped[str] = mapped_column(String(60), default="", nullable=False, index=True)
    agent: Mapped[str] = mapped_column(String(60), default="", nullable=False, index=True)
    reasoning_mode: Mapped[str] = mapped_column(String(40), default="", nullable=False, index=True)
    ...
    target_id: Mapped[str] = mapped_column(String(120), default="", nullable=False, index=True)
    input: Mapped[dict] = mapped_column(JSON, default=dict, nullable=False)
    result: Mapped[dict] = mapped_column(JSON, default=dict, nullable=False)
    error: Mapped[str] = mapped_column(Text, default="", nullable=False)
    timing_ms: Mapped[int] = mapped_column(Integer, default=0, nullable=False)
    usage: Mapped[dict] = mapped_column(JSON, default=dict, nullable=False)


class PluginAccessToken(Base, TimestampMixin):
    __tablename__ = "plugin_access_tokens"

    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    workspace_id: Mapped[int] = mapped_column(ForeignKey("workspaces.id"), nullable=False, index=True)
    created_by_id: Mapped[int | None] = mapped_column(ForeignKey("users.id"), nullable=True, index=True)
    name: Mapped[str] = mapped_column(String(160), default="", nullable=False)
    token_prefix: Mapped[str] = mapped_column(String(32), default="", nullable=False, index=True)
