...
def _ensure_asset_constraints(db: Session) -> None:
    ...
    if dialect == "sqlite":
        ...
        # 且 create_all 已经处理了新表。旧表迁移通常需要重建表，
        # 在这个轻量级项目中，我们依赖 create_all 处理首次创建。
        pass
    else:
        # PostgreSQL
...
def _ensure_demo_data(db: Session, workspace: Workspace, admin: User) -> None:
    ...
    if not project:
        project = Project(workspace_id=workspace.id, name="演示项目-攻防演练", description="用于快速验证日志解析、资产联动、告警协作和导出功能。")
        db.add(project)
        db.flush()
    ...
    weblogic_asset = _ensure_asset(
        ...
        workspace.id,
        {
            ...
            "description": "用于验证态势感知原始日志解析和资产命中。",
        },
    )
