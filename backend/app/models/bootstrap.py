...
def _ensure_alert_columns(db: Session) -> None:
    ...
    if dialect == "sqlite":
        table_columns = {}
        for table in ("alerts", "templates", "parse_rules", "settings", "devices", "users"):
        ...
        if "source_context" not in table_columns.get("alerts", {}):
            db.execute(text("ALTER TABLE alerts ADD COLUMN source_context JSON DEFAULT '{}' NOT NULL"))
        ...
        for col, col_type in [
            ...
            ("reported_by_name", "VARCHAR(120) DEFAULT '' NOT NULL"),
            ("analysis_result", "VARCHAR(120) DEFAULT '' NOT NULL"),
            ("is_emergency", "BOOLEAN DEFAULT 0 NOT NULL"),
            ("block_device_ids", "JSON DEFAULT '[]' NOT NULL"),
            ("block_at", "DATETIME"),
        ...
        if "device_role" not in table_columns.get("devices", {}):
            db.execute(text("ALTER TABLE devices ADD COLUMN device_role VARCHAR(40) DEFAULT 'monitor' NOT NULL"))
        if "browser_assistant_enabled" not in table_columns.get("devices", {}):
            db.execute(text("ALTER TABLE devices ADD COLUMN browser_assistant_enabled BOOLEAN DEFAULT 1 NOT NULL"))
        if "browser_url_patterns" not in table_columns.get("devices", {}):
            db.execute(text("ALTER TABLE devices ADD COLUMN browser_url_patterns JSON DEFAULT '[\"http://*/*\",\"https://*/*\"]' NOT NULL"))
        ...
        if "roles" not in table_columns.get("users", {}):
            db.execute(text("ALTER TABLE users ADD COLUMN roles JSON DEFAULT '[]' NOT NULL"))
    else:
        # PostgreSQL
        for table, col, col_type in [
            ...
            ("alerts", "source_context", "JSON DEFAULT '{}'::json NOT NULL"),
            ...
            ("alerts", "reported_by_name", "VARCHAR(120) DEFAULT '' NOT NULL"),
            ("alerts", "analysis_result", "VARCHAR(120) DEFAULT '' NOT NULL"),
            ("alerts", "is_emergency", "BOOLEAN DEFAULT FALSE NOT NULL"),
            ("alerts", "block_device_ids", "JSON DEFAULT '[]'::json NOT NULL"),
            ("alerts", "block_at", "TIMESTAMP"),
            ("alerts", "response_note", "TEXT DEFAULT '' NOT NULL"),
            ("alerts", "response_owner_id", "INTEGER"),
            ...
            ("devices", "device_role", "VARCHAR(40) DEFAULT 'monitor' NOT NULL"),
            ("devices", "browser_assistant_enabled", "BOOLEAN DEFAULT TRUE NOT NULL"),
            ("devices", "browser_url_patterns", "JSON DEFAULT '[\"http://*/*\",\"https://*/*\"]'::json NOT NULL"),
            ...
            ("users", "roles", "JSON DEFAULT '[]'::json NOT NULL")
        ]:
            check_sql = text(f"""
...
def _backfill_alert_workflow_fields(db: Session) -> None:
    ...
    for row in rows:
        if not row.reported_by_name:
            if row.created_by_id:
                creator = db.get(User, row.created_by_id)
                row.reported_by_name = creator.display_name if creator else ""
            else:
                row.reported_by_name = ""
        if not row.analysis_result and row.disposal_target and row.disposal_action:
            row.analysis_result = f"{DISPOSAL_TARGET_LABELS.get(row.disposal_target, row.disposal_target)}{DISPOSAL_ACTION_LABELS.get(row.disposal_action, row.disposal_action)}"
        row.is_emergency = bool(row.disposal_action == "emergency")
...
def _rename_template_if_needed(db: Session, workspace_id: int, old_name: str, new_name: str) -> None:
    row = db.query(Template).filter_by(workspace_id=workspace_id, name=old_name).first()
    if not row:
        return
    existing = db.query(Template).filter_by(workspace_id=workspace_id, name=new_name).first()
...
def _ensure_security_tracking_template(db: Session, workspace: Workspace) -> None:
    _rename_template_if_needed(db, workspace.id, "ISOP-安全事件跟踪表-CSV", "安全事件跟踪表")
    _ensure_template(
        db,
        workspace.id,
        {
            ...
            "variables": [
                "告警编号", "事件ID", "告警时间", "告警设备", "告警设备IP地址", "告警名称", "攻击源IP", "攻击目的IP",
                "监测上报人员", "研判人员", "是否应急", "研判结果", "封禁位置", "封禁人员", "封禁时间", "处置描述", "应急人员",
            ],
            "scope": "team",
            "is_default": True,
        },
...
def _ensure_demo_data(db: Session, workspace: Workspace, admin: User, settings) -> None:
    ...
    for item in demo_alerts:
        ...
        db.add(
            Alert(
                ...
                claimed_at=now(db, workspace.id) if item.get("assignee") else None,
...
def bootstrap_defaults(db: Session) -> None:
    ...
    if not user:
        ...
        user.roles = [user.role or "admin"]

    for existing_user in db.query(User).filter(User.workspace_id == workspace.id).all():
        if not existing_user.roles:
            existing_user.roles = [existing_user.role or "viewer"]
