...
from app.api.deps import current_user, has_role, require_admin, require_not_viewer
from app.models.database import SessionLocal, get_db
from app.models.entities import AiConversation, AiExperience, AiMemory, AiMessage, AiPrompt, AiRun, Alert, User
...
from app.services.ai_agent import stream_chat_agent, safe_sse_event, update_conversation_memory
...
def _can_manage_experience(user: User) -> bool:
    return has_role(user, "admin") or has_role(user, "analyst") or has_role(user, "disposer")


def _can_publish_experience(user: User) -> bool:
    return has_role(user, "admin") or has_role(user, "analyst")
...
def list_experiences(
    ...
    if not _can_manage_experience(user):
...
def update_experience(experience_id: int, payload: AiExperienceUpdate, db: Session = Depends(get_db), user: User = Depends(require_not_viewer)):
    ...
    if not _can_manage_experience(user) and row.created_by_id != user.id:
...
def publish_experience(experience_id: int, db: Session = Depends(get_db), user: User = Depends(require_not_viewer)):
    ...
    if not row or row.workspace_id != user.workspace_id:
        raise HTTPException(status_code=404, detail="经验不存在")
    if not _can_publish_experience(user):
...
def extract_experience(payload: AiExperienceExtractRequest, db: Session = Depends(get_db), user: User = Depends(require_not_viewer)):
    ...
    if payload.save or payload.publish:
        status = "draft"
        if payload.publish:
            if _can_publish_experience(user):
                status = "published"
            else:
...
def _get_conversation(db: Session, user: User, conversation_id: int) -> AiConversation:
    ...
    if not has_role(user, "admin") and row.created_by_id != user.id:
...
def delete_conversation(conversation_id: int, db: Session = Depends(get_db), user: User = Depends(require_not_viewer)):
    ...
    db.query(AiMemory).filter_by(workspace_id=user.workspace_id, conversation_id=row.id).delete()
    db.delete(row)
    db.commit()
    return {"ok": True}
...
async def send_chat_message(conversation_id: int, payload: AiChatRequest, db: Session = Depends(get_db), user: User = Depends(require_not_viewer)):
    ...
    async def event_generator():
        ...
        try:
            ...
            async for raw_data in stream_chat_agent(
                stream_db,
                stream_user,
                conv_id,
                payload.content,
            ...
            if final_answer:
                try:
                    ...
                    ai_settings = get_effective_setting(stream_db, workspace_id, user_id, "ai")
                except Exception:
                    ai_settings = {}
                ai_msg = AiMessage(
                    ...
                    tool_calls=[
                        ...
                        {"tool": "agent_meta", "data": {"agent": payload.agent, "reasoning_mode": payload.reasoning_mode}}
                    ],
                    created_by_id=user_id
                )
                stream_db.add(ai_msg)
                try:
                    update_conversation_memory(stream_db, stream_user, conv_id, payload.agent, payload.content, final_answer, ai_settings or {})
                except Exception:
                    pass
                stream_db.add(AiRun(
                    workspace_id=workspace_id,
                    ...
                    reasoning_mode=payload.reasoning_mode,
                    status="success",
                    model=str((ai_settings or {}).get("model") or ""),
                    target_type="ai_conversation",
                    target_id=str(conv_id),
                    input={"question": payload.content},
                    result={"answer": final_answer, "trace": full_trace, "evidences": evidences},
