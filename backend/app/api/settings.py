...
from app.api.deps import current_user, has_role, require_not_viewer
from app.core.timezone import COMMON_TIMEZONES, SYSTEM_TIME_KEY, get_system_time_config, is_valid_timezone, now, normalize_system_time
...
def update_setting(
    ...
    # 权限校验
    if scope == "global" and not has_role(user, "admin"):
        raise HTTPException(status_code=403, detail="仅管理员可修改全员配置")
    if key == SYSTEM_TIME_KEY:
        if scope != "global" or not has_role(user, "admin"):
            raise HTTPException(status_code=403, detail="仅管理员可修改系统时间配置")
        payload.value = normalize_system_time(payload.value)
        if not is_valid_timezone(payload.value["timezone"]):
    ...
    if not row:
        ...
        row = Setting(
            ...
            value=new_value,
            created_at=now(db, user.workspace_id),
            updated_at=now(db, user.workspace_id),
        )
        db.add(row)
    else:
        # 乐观锁检查
        ...
        row.updated_at = now(db, user.workspace_id)
...
@router.get("/system-time", response_model=dict[str, Any])
def system_time_info(db: Session = Depends(get_db), user: User = Depends(current_user)):
    config = get_system_time_config(db, user.workspace_id)
    ...
    return {
        ...
        "app_time": now(db, user.workspace_id).isoformat(sep=" ", timespec="seconds"),
        "server_time": server_now.isoformat(sep=" ", timespec="seconds"),
        "server_local_time": server_now.isoformat(sep=" ", timespec="seconds"),
        "utc_time": utc_now.isoformat(sep=" ", timespec="seconds"),
