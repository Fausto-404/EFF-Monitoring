...
def delete_task(task_id: int, db: Session = Depends(get_db), user: User = Depends(require_admin)):
    ...
    db.delete(row)
    write_audit(db, user, "task.delete", "task", task_id, {"deleted": 1})
    db.commit()
    return {"ok": True, "deleted": 1}


@router.post("/tasks/batch-delete")
def batch_delete_tasks(payload: IdsRequest, db: Session = Depends(get_db), user: User = Depends(require_admin)):
    ids = list(dict.fromkeys(payload.ids))
    if not ids:
        raise HTTPException(status_code=400, detail="请选择要删除的任务记录")
    deleted = db.query(TaskRecord).filter(
        TaskRecord.workspace_id == user.workspace_id,
        TaskRecord.id.in_(ids),
    ).delete(synchronize_session=False)
    write_audit(db, user, "task.batch_delete", "task", ",".join(str(item) for item in ids), {"requested": len(ids), "deleted": deleted})
    db.commit()
