...
def test_webhook(payload: WebhookTestRequest, db: Session = Depends(get_db), user: User = Depends(current_user)):
    return _send_webhook_text(payload, db, user, "webhook.test")


@router.post("/webhook/send")
def send_webhook(payload: WebhookTestRequest, db: Session = Depends(get_db), user: User = Depends(current_user)):
