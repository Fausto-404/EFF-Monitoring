...
def extract_ste_experience(db: Session, user: User, alert: Alert) -> dict[str, Any]:
    ...
    if not data:
        # 记录原始输出以便调试（如果环境允许）
        print(f"DEBUG: AI failed to return valid STE JSON for alert {alert.id}. Raw content: {content[:200]}...")
        raise HTTPException(
            status_code=500, 
...
def handle_auto_ste_task(db: Session, task: TaskRecord) -> dict[str, Any]:
    ...
    try:
        ...
        print(f"DEBUG: Auto-STE failed for alert {alert_id}: {str(e)}")
        raise e
...
def available_template_variable_catalog(db: Session, user: User, device_id: int | None) -> list[dict[str, str]]:
    rules = db.query(ParseRule).filter(ParseRule.workspace_id == user.workspace_id, ParseRule.enabled.is_(True)).all()
    items = [
        {"name": r.name, "field_key": r.field_key or "", "field_label": r.field_label or ""}
        for r in rules
        if not r.device_id or not device_id or r.device_id == device_id
    ]
    system_items = [
        {"name": name, "field_key": key, "field_label": label}
        for name, key, label in [
            ("告警ID", "alert_code", "告警ID"),
            ("告警Hash", "alert_hash", "告警Hash"),
            ("状态", "status", "状态"),
            ("所属组", "current_group", "所属组"),
            ("设备名称", "device_name", "设备名称"),
            ("当前时间", "current_time", "当前时间"),
            ("当前日期", "current_date", "当前日期"),
            ("源资产名称", "src_asset_name", "源资产名称"),
            ("源资产区域", "src_asset_area", "源资产区域"),
            ("源资产负责人", "src_asset_owner", "源资产负责人"),
            ("源资产重要性", "src_asset_criticality", "源资产重要性"),
            ("目的资产名称", "dst_asset_name", "目的资产名称"),
            ("目的资产区域", "dst_asset_area", "目的资产区域"),
            ("目的资产负责人", "dst_asset_owner", "目的资产负责人"),
            ("目的资产重要性", "dst_asset_criticality", "目的资产重要性"),
            ("AI 研判结果", "ai_result", "AI 研判结果"),
            ("威胁情报结果", "ti_result", "威胁情报结果"),
        ]
    ]
