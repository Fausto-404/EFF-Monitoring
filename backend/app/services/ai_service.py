...
def plan_alert_analysis(db: Session, user: User, pack: dict[str, Any], matches: list[tuple[AiExperience, int]], timeout: int = 45) -> dict[str, Any]:
...
def reflect_alert_analysis(db: Session, user: User, draft: str, pack: dict[str, Any], matches: list[tuple[AiExperience, int]], plan: dict[str, Any], timeout: int = 60) -> str:
...
def _generate_alert_analysis_text(
    db: Session,
    user: User,
    alert: Alert,
    *,
    ...
    plan = plan_alert_analysis(db, user, pack, matches, timeout=planner_timeout)
    runtime_evidences = [
    ...
    try:
            ...
            [
                {
                    ...
                    "content": (
                        ...
                        "输出结构：研判标签、结论、关键证据、风险判断、证据链分析、处置建议、不确定性、引用经验。"
                        "研判标签只能从：业务行为、存在攻击意图、攻击失败、攻击成功、无法确认 中选择。"
                        "标签互斥规则：业务行为不能与攻击类标签共存；攻击成功和攻击失败互斥；成功/失败必须同时包含存在攻击意图。"
                    ),
                },
                {
                    "role": "user",
        ...
        final = reflect_alert_analysis(db, user, draft, pack, matches, {**plan, "tool_evidences": runtime_evidences}, timeout=reflect_timeout)
    except HTTPException as exc:
        summaries = "\n".join(f"- {item.get('summary')}" for item in runtime_evidences if item.get("summary"))
        final = f"AI 研判失败：{exc.detail}\n\n已完成平台证据查询：\n{summaries}"
    matched_ids = [item.knowledge_id for item, _score in matches]
    return final, matched_ids, runtime_evidences


def analyze_alert_with_experience(db: Session, user: User, alert: Alert) -> tuple[str, list[str]]:
    result, matched_ids = investigate_threat(db, user, source="alert", alert=alert)
    return render_v220_alert_analysis(result), matched_ids
...
def investigate_threat(
    ...
    *,
    ...
    text, matched_ids, runtime_evidences = _generate_alert_analysis_text(
        db,
        user,
        alert,
    ...
    result = build_threat_analysis_result(
        text,
        matched_ids,
        runtime_evidences,
        source=source,
...
VERDICT_LABELS = ["业务行为", "存在攻击意图", "攻击失败", "攻击成功", "非告警事件", "无法确认"]


def _normalize_verdict_labels(text: str, source: str) -> list[str]:
    explicit = re.search(
        r"研判标签\s*[:：]?\s*(?:\n|\r|\s|[#*`_-])*(业务行为|存在攻击意图|攻击失败|攻击成功|非告警事件|无法确认)",
        text,
    )
    if explicit:
        labels = [explicit.group(1)]
    else:
        labels = [label for label in VERDICT_LABELS if label in text]
    lower = text.lower()
    if not labels:
        if any(token in text for token in ["拦截", "阻断", "403", "失败", "未成功", "未命中"]):
            labels = ["存在攻击意图", "攻击失败"]
        elif any(token in text for token in ["成功", "执行", "WebShell", "敏感数据", "失陷"]) or "success" in lower:
            labels = ["存在攻击意图", "攻击成功"]
        elif any(token in text for token in ["攻击", "扫描", "漏洞", "payload", "载荷", "恶意", "CVE"]):
            labels = ["存在攻击意图", "无法确认"]
        elif any(token in text for token in ["业务", "正常", "授权", "白名单"]):
            labels = ["业务行为"]
        else:
            labels = ["无法确认"]
    labels = list(dict.fromkeys(labels))
    label_set = set(labels)
    if "攻击成功" in label_set and "攻击失败" in label_set:
        return ["存在攻击意图", "无法确认"]
    if "业务行为" in label_set:
        return ["业务行为"]
    if "攻击成功" in label_set:
        return ["存在攻击意图", "攻击成功"]
    if "攻击失败" in label_set:
        return ["存在攻击意图", "攻击失败"]
    if "非告警事件" in label_set and source == "plugin":
        return ["非告警事件"]
    if "存在攻击意图" in label_set:
        return ["存在攻击意图", "无法确认"] if "无法确认" in label_set else ["存在攻击意图"]
    return ["无法确认"]


def _risk_from_text(text: str) -> str:
    if any(token in text for token in ["极高", "critical", "严重失陷"]):
        return "critical"
    if any(token in text for token in ["高危", "high", "攻击成功", "失陷", "命令执行"]):
        return "high"
    if any(token in text for token in ["中危", "medium", "横向", "漏洞利用"]):
        return "medium"
    if any(token in text for token in ["低危", "low", "攻击失败", "拦截"]):
        return "low"
    return "未知"


def _extract_analysis_conclusion(text: str) -> str:
    body = (text or "").strip()
    if not body:
        return "无法形成稳定结论"
    section = re.search(r"(?:^|\n)\s*#{0,4}\s*结论\s*[:：]?\s*(.*?)(?=\n\s*#{1,4}\s*\S|$)", body, re.S)
    ...
    for line in body.splitlines():
        cleaned = line.strip(" -#*`：:")
        if cleaned and cleaned not in {"研判标签", "结论", "关键证据", "风险判断"} and cleaned not in VERDICT_LABELS:
            return cleaned[:180]
    return "无法形成稳定结论"


def _analysis_steps(text: str, runtime_evidences: list[dict[str, Any]]) -> list[dict[str, str]]:
    evidence_summary = "；".join(str(item.get("summary") or "") for item in runtime_evidences if item.get("summary"))[:220]
    return [
        {"title": "平台证据增强", "status": "已完成", "detail": evidence_summary or "已完成告警详情、时间线、资产和相似告警查询。"},
        {"title": "攻击特征判断", "status": "已分析", "detail": text[:220] if text else "AI 网关未配置或未返回研判文本。"},
        {"title": "综合结论", "status": "已规范化", "detail": "已按业务行为、攻击意图、攻击成功/失败、无法确认等标签进行互斥归一。"},
    ]


def build_threat_analysis_result(
    text: str,
    matched_ids: list[str],
    runtime_evidences: list[dict[str, Any]],
    *,
    source: str,
    include_recommended_actions: bool,
) -> dict[str, Any]:
    body = (text or "").strip()
    labels = _normalize_verdict_labels(body, source)
    conclusion = "AI 网关未配置" if "AI 网关未配置" in body else _extract_analysis_conclusion(body)
    result = {
        "analysis_mode": "smart",
        "evidence_scope": "platform_enriched",
        "verdict_labels": labels,
        "conclusion": conclusion,
        "confidence": 70 if "无法确认" not in labels and body else 45,
        "risk_level": _risk_from_text(body),
        "summary": body,
        "analysis_steps": _analysis_steps(body, runtime_evidences),
        "evidence": [str(item.get("summary") or item.get("tool") or "") for item in runtime_evidences if item],
        "referenced_experiences": matched_ids,
        "uncertainties": ["AI 网关未配置，未执行模型研判。"] if "AI 网关未配置" in body else [],
        "recommended_actions": [],
    }
    if include_recommended_actions:
        result["recommended_actions"] = ["结合关键证据复核源/目的资产日志。", "根据研判结果执行封禁、忽略或闭环流转。"]
    return result


def render_v220_alert_analysis(result: dict[str, Any]) -> str:
    refs = "、".join(result.get("referenced_experiences") or []) if result.get("referenced_experiences") else "无"
    labels = "、".join(result.get("verdict_labels") or []) or "无法确认"
    actions = "\n".join(f"- {item}" for item in result.get("recommended_actions") or [])
    return (
        "【v2.2.0 平台告警智能研判】\n"
        "研判模式：智能研判（平台证据增强）\n"
        "证据范围：当前告警 + 解析字段 + 资产上下文 + 相似告警 + STE 历史经验\n"
        f"研判标签：{labels}\n"
        f"风险等级：{result.get('risk_level') or '未知'}\n"
        f"置信度：{result.get('confidence') if result.get('confidence') is not None else '未知'}\n"
        f"引用经验：{refs}\n\n"
        f"结论：{result.get('conclusion') or '无法形成稳定结论'}\n\n"
        f"摘要：\n{result.get('summary') or ''}\n\n"
        "证据链：\n"
        + "\n".join(f"- {item.get('title')}：{item.get('status')}，{item.get('detail')}" for item in result.get("analysis_steps") or [])
...
def available_template_variable_catalog(db: Session, user: User, device_id: int | None) -> list[dict[str, Any]]:
    ...
    system_items = [
        ...
        for name, key, label, domain, desc in [
            ...
            ("监测上报人员", "reported_by_name", "监测上报人员", "alert", "同步或创建该告警的人员"),
            ("研判结果", "analysis_result", "研判结果", "alert", "研判阶段给出的处置建议，如目的 IP应急"),
            ("是否应急", "is_emergency", "是否应急", "alert", "该告警是否被研判为应急动作"),
            ("封禁位置", "block_device_names", "封禁位置", "alert", "处置完成时实际选择的封禁设备名称"),
            ("封禁时间", "block_at", "封禁时间", "alert", "封禁类处置完成时间"),
            ("处置描述", "response_note", "处置描述", "alert", "应急处置完成时填写的说明"),
            ("应急人员", "response_owner", "应急人员", "alert", "应急处置完成时选择的人员"),
    ...
    for call in calls[:12]:
        ...
        if tool == "get_asset_by_ip":
            ...
            builtin_vars = [
                ...
                {"name": "监测上报人员", "field_key": "reported_by_name", "label": "监测上报人员", "type": "builtin"},
                {"name": "研判结果", "field_key": "analysis_result", "label": "研判结果", "type": "builtin"},
                {"name": "是否应急", "field_key": "is_emergency", "label": "是否应急", "type": "builtin"},
                {"name": "封禁位置", "field_key": "block_device_names", "label": "封禁位置", "type": "builtin"},
                {"name": "封禁时间", "field_key": "block_at", "label": "封禁时间", "type": "builtin"},
                {"name": "处置描述", "field_key": "response_note", "label": "处置描述", "type": "builtin"},
                {"name": "应急人员", "field_key": "response_owner", "label": "应急人员", "type": "builtin"},
            ...
            results.append({"tool": tool, "params": params, "data": [{"id": r.id, "name": r.name, "vendor": r.vendor, "product": r.product, "device_ip": r.version, "version": r.version} for r in rows]})
