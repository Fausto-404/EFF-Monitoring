...
from app.api.deps import has_role, has_any_role, user_roles
...
def device_search(db: Session, user: User, params: dict[str, Any]) -> dict[str, Any]:
    ...
    items = [{"id": r.id, "name": r.name, "vendor": r.vendor, "product": r.product, "device_ip": r.version, "version": r.version} for r in rows.order_by(Device.updated_at.desc()).limit(30).all()]
...
def user_public_search(db: Session, user: User, params: dict[str, Any]) -> dict[str, Any]:
    ...
    if role:
        rows = [item for item in rows.all() if has_role(item, role)]
    else:
        rows = rows.all()
    if q:
        q_lower = q.lower()
        rows = [
            ...
            if q_lower in (item.username or "").lower()
            or q_lower in (item.display_name or "").lower()
            or any(q_lower in (ROLE_LABELS.get(role, role)).lower() for role in user_roles(item))
        ]
    rows = sorted(rows, key=lambda item: item.updated_at, reverse=True)[:50]
    items = [{"id": r.id, "username": r.username, "display_name": r.display_name, "roles": [ROLE_LABELS.get(role, role) for role in user_roles(r)], "role": ROLE_LABELS.get(r.role, r.role), "is_active": r.is_active} for r in rows]
...
def ops_summary(db: Session, user: User, params: dict[str, Any]) -> dict[str, Any]:
    since = now(db, user.workspace_id) - timedelta(days=7)
...
def experience_search(db: Session, user: User, params: dict[str, Any]) -> dict[str, Any]:
    ...
    if not has_any_role(user, {"admin", "analyst", "disposer"}):
...
def evidence_extract_ioc(db: Session, user: User, params: dict[str, Any]) -> dict[str, Any]:
    ...
    ips = sorted(set(re.findall(r"(?<![\d.])(?:\d{1,3}\.){3}\d{1,3}(?![\d.])", text)))
...
def agent_tool_registry(db: Session, user: User, params: dict[str, Any]) -> dict[str, Any]:
    ...
    for name, meta in TOOL_REGISTRY.items():
        if has_any_role(user, meta["roles"]):
            tool_info = {
                "tool": name,
                "level": meta["level"],
...
def system_permissions(db: Session, user: User, params: dict[str, Any]) -> dict[str, Any]:
    available_tools = [name for name, meta in TOOL_REGISTRY.items() if has_any_role(user, meta["roles"])]
    restricted_tools = [name for name, meta in TOOL_REGISTRY.items() if not has_any_role(user, meta["roles"])]
...
def execute_tool(db: Session, user: User, tool_name: str, params: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if not has_any_role(user, meta["roles"]):
