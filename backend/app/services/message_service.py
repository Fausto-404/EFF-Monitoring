...
def create_message(
    ...
    *,
    ...
    row = Message(
        ...
        payload=payload or {},
        created_at=now(db, recipient.workspace_id),
        updated_at=now(db, recipient.workspace_id),
    )
    db.add(row)
    return row
...
def notify_role(
    ...
    *,
    ...
    users = [
        item
        for item in db.query(User).filter_by(workspace_id=workspace_id, is_active=True).all()
...
def mark_message_read(db: Session, message: Message) -> None:
    if not message.is_read:
        ...
        message.read_at = now(db, message.workspace_id)
