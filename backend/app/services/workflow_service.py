...
from app.models.entities import Alert, User, AiExperience, Device
...
def _create_pending_experience(db: Session, alert: Alert):
    ...
    row = AiExperience(
        ...
        knowledge_id=f"EXP-{now(db, alert.workspace_id).strftime('%Y%m%d%H%M')}-{alert.id}",
...
def can_claim(user: User, alert: Alert) -> bool:
    ...
    return has_role(user, ACTIVE_GROUP_ROLE.get(alert.current_group) or "") and not alert.assignee_id


def claim_alert(db: Session, user: User, alert: Alert, *, updated_at: datetime | None = None) -> Alert:
    # 行级锁防并发认领
    locked = db.query(Alert).filter(Alert.id == alert.id).with_for_update().first()
    if locked:
        for attr in ["assignee_id", "current_group", "status", "updated_at", "analysis_owner_id", "disposal_owner_id"]:
            setattr(alert, attr, getattr(locked, attr))
    _ensure_current_status(alert)
    ...
    if not has_role(user, ROLE_ADMIN) and not has_role(user, ACTIVE_GROUP_ROLE.get(alert.current_group) or ""):
        raise HTTPException(status_code=403, detail="当前账号不属于该告警所属组")

    before = _snapshot(alert)
    alert.assignee_id = user.id
    alert.claimed_at = now(db, alert.workspace_id)
    ...
    alert.updated_at = now(db, alert.workspace_id)
...
def release_claim(
    ...
    *,
    ...
    locked = db.query(Alert).filter(Alert.id == alert.id).with_for_update().first()
    if locked:
        for attr in ["assignee_id", "claimed_at", "current_group", "status", "updated_at"]:
            setattr(alert, attr, getattr(locked, attr))
    _ensure_current_status(alert)
    _assert_unstale(alert, updated_at)
    if alert.current_group not in ACTIVE_GROUP_ROLE:
    ...
    if not force and not has_role(user, ROLE_ADMIN) and alert.assignee_id != user.id:
        raise HTTPException(status_code=403, detail="只能释放自己认领的告警")
    if force and not has_role(user, ROLE_ADMIN):
        raise HTTPException(status_code=403, detail="需要管理员权限")

    before = _snapshot(alert)
    alert.assignee_id = assign_user_id
    alert.claimed_at = now(db, alert.workspace_id) if assign_user_id else None
...
def assign_alert(db: Session, user: User, alert: Alert, assignee: User, *, updated_at: datetime | None = None) -> Alert:
    ...
    if assignee.workspace_id != alert.workspace_id or not assignee.is_active or not has_role(assignee, expected_role):
...
def _ensure_claim_for_transition(user: User, alert: Alert, target_status: str) -> None:
    ...
    if has_role(user, ROLE_ANALYST):
        if alert.current_group == GROUP_ANALYSIS and alert.assignee_id == user.id and target_status in {STATUS_FALSE_POSITIVE, STATUS_IGNORED, STATUS_DISPOSAL}:
            return
    if has_role(user, ROLE_DISPOSER):
...
def _resolve_block_devices(db: Session, workspace_id: int, block_device_ids: list[int]) -> list[Device]:
    ids = [int(item) for item in block_device_ids if item]
    if not ids:
        return []
    rows = db.query(Device).filter(Device.workspace_id == workspace_id, Device.id.in_(ids)).all()
    if len(rows) != len(set(ids)):
        raise HTTPException(status_code=400, detail="存在无效的封禁设备")
    invalid = [row.name for row in rows if row.device_role != "block"]
    if invalid:
        raise HTTPException(status_code=400, detail=f"以下设备不是封禁设备：{', '.join(invalid)}")
    order = {device_id: idx for idx, device_id in enumerate(ids)}
...
def transition_alert(
    ...
    *,
    target_status: str,
    block_device_ids: list[int] | None = None,
    block_at: datetime | None = None,
    response_note: str = "",
    response_owner_id: int | None = None,
    disposal_target: str = "",
    disposal_action: str = "",
    closure_target: str = "",
    closure_action: str = "",
    ...
    locked = db.query(Alert).filter(Alert.id == alert.id).with_for_update().first()
    if locked:
        for attr in ["status", "current_group", "assignee_id", "claimed_at", "updated_at",
                      "analysis_owner_id", "disposal_owner_id", "disposal_target", "disposal_action",
                      "disposal_ip", "severity", "block_device_ids", "block_at", "response_note",
                      "response_owner_id", "closure_target", "closure_action", "false_positive_reason"]:
            setattr(alert, attr, getattr(locked, attr))
    _ensure_current_status(alert)
    _assert_unstale(alert, updated_at)
    ...
    if target_status == STATUS_DISPOSAL:
        ...
        alert.analysis_result = f"{DISPOSAL_TARGET_LABELS.get(disposal_target, disposal_target)}{DISPOSAL_ACTION_LABELS.get(disposal_action, disposal_action)}"
        alert.is_emergency = disposal_action == "emergency"
        alert.block_device_ids = []
        alert.block_at = None
        alert.response_note = ""
        alert.response_owner_id = None
        ...
        if has_role(user, ROLE_ANALYST) or has_role(user, ROLE_ADMIN):
            alert.analysis_owner_id = user.id
        ...
        if target_status == STATUS_DISPOSED and previous_group == GROUP_DISPOSAL:
            if alert.disposal_action == "block":
                selected_block_devices = _resolve_block_devices(db, alert.workspace_id, block_device_ids or [])
                if not selected_block_devices:
                    raise HTTPException(status_code=400, detail="封禁类处置完成时必须选择至少一个封禁设备")
                if not block_at:
                    raise HTTPException(status_code=400, detail="封禁类处置完成时必须填写封禁时间")
                alert.block_device_ids = [row.id for row in selected_block_devices]
                alert.block_at = block_at
                alert.response_note = ""
                alert.response_owner_id = None
            elif alert.disposal_action == "emergency":
                if response_owner_id:
                    response_owner = db.get(User, response_owner_id)
                    if not response_owner or response_owner.workspace_id != alert.workspace_id or not response_owner.is_active:
                        raise HTTPException(status_code=400, detail="请选择有效的应急人员")
                alert.response_note = response_note.strip()
                alert.response_owner_id = response_owner_id
                alert.block_device_ids = []
                alert.block_at = None
            else:
                alert.block_device_ids = []
