...
ROLE_TO_EVIDENCE_TYPES = {
    ...
    "risk_signals": {"ip": ["alert_list", "similar_alerts", "raw_log_match"], "alert_hash": ["alert_detail", "alert_timeline"], "asset": ["alert_list", "asset_risk_context"], "*": ["alert_list"]},
    "external_intel": {"ip": ["ip_reputation", "ip_intel_report"], "*": ["ip_reputation"]},
    "internal_context": {"*": ["asset_profile", "device_profile", "project_list"]},
    ...
    "historical_cases": {"*": ["ste_experience", "similar_case"]},
    "tabular_data": {"alert": ["alert_list"], "audit": ["audit_timeline"], "task": ["task_status"], "*": ["tabular_dataset", "alert_list"]},
...
def score_tool_candidate(tool_schema: dict, required_evidence_types: list[str], evidence_roles: list[str], entities: dict, params: dict, state: dict) -> dict:
    ...
    if not gain and not blocked:
        score -= 5
        reasons.append("没有覆盖必须的证据")
...
def select_tools_by_evidence_advanced(task_model: dict, required_evidence_types: list[str], tool_schemas: list[dict], state: dict) -> list[dict]:
    ...
    for tool in tool_schemas:
        if tool["tool"] in already_called:
            continue
    ...
    for score, tool_name, reasons in candidate_scores:
        ...
        if tool_name.startswith("analysis."):
            if not has_data_source and not any("tabular_dataset" in next(t for t in tool_schemas if t["tool"] == s).get("output_evidence_types", []) or "alert_list" in next(t for t in tool_schemas if t["tool"] == s).get("output_evidence_types", []) for s in selected_tools):
                continue 
                
        if provides.intersection(missing_ev) or tool_name.startswith("analysis."): 
...
def build_default_tool_params(tool_name: str, task_model: dict, entities: dict, state: dict, evidence_packs: list[dict]) -> dict:
    ...
    if tool_name in ("asset.get_by_ip", "asset.segment_match", "ti.lookup_ip", "intel.ip_report"):
        ...
        if ips:
            if "源" in state.get("question", ""):
                params["ip"] = ips[0]
                params["ip_match"] = "source"
            elif "目的" in state.get("question", ""):
                params["ip"] = ips[0]
                params["ip_match"] = "destination"
            else:
                params["ip"] = ips[0]
        ...
        if "告警数" in question or "多少告警" in question:
            params["metrics"] = [{"name": "alert_count", "operation": "count_distinct", "field": "alert_hash"}]
    elif tool_name == "analysis.aggregate":
        params["operation"] = "count_distinct"
        params["field"] = "alert_hash"
    elif tool_name == "analysis.timeseries":
        params["time_field"] = "created_at"
        params["interval"] = "day"
        params["metrics"] = [{"name": "count", "operation": "count_distinct", "field": "alert_hash"}]
    elif tool_name == "analysis.topn":
        question = state.get("question", "")
        if "源 IP" in question:
            params["group_by"] = "src_ip"
            ...
            params["group_by"] = "dst_ip"
        elif "处理人" in question or "谁" in question:
            params["group_by"] = "assignee"
        else:
            params["group_by"] = "event_type"
            
        params["metric"] = {"name": "count", "operation": "count_distinct", "field": "alert_hash"}
        params["limit"] = 10
    elif tool_name == "analysis.duration":
        params["start_field"] = "created_at"
        params["end_field"] = "closed_at"
    ...
    while state["tool_repair_rounds"] <= 2:
        ...
        if coverage["sufficient"]:
            ...
            if state["tool_repair_rounds"] > 2:
                ...
                break
            yield json.dumps({"event": "trace", "data": f"证据不足，缺失：{', '.join(coverage['missing'])}，准备第 {state['tool_repair_rounds']} 轮补查..."})
