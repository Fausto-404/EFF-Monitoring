...
echo
echo "管理员账号：$(sed -n 's/^INITIAL_ADMIN_USERNAME=//p' .env | tail -1)"
echo "管理员密码：$(sed -n 's/^INITIAL_ADMIN_PASSWORD=//p' .env | tail -1)"
echo "提示：以上为 .env 中配置的初始密码；如果已在系统内修改密码，请使用修改后的密码登录。"
echo "密码同时保存在当前目录 .env 中，请妥善保管。"
if grep -q '^INITIAL_ADMIN_PASSWORD=admin123$' .env 2>/dev/null; then
  echo "警告：当前仍使用默认管理员密码 admin123，请登录后立即修改。"
fi
