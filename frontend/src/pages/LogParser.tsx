...
import type { Device, Template, User } from '../api/types';
import HelpTip from '../components/HelpTip';
import ReportGenerateModal from '../components/ReportGenerateModal';
import { hasRole, isOnlyViewer } from '../utils/roles';
...
const currentParserStorageScope = () => {
  const token = localStorage.getItem('eff_token') || '';
  try {
    const payload = JSON.parse(atob(token.split('.')[1] || ''));
    return payload?.sub ? `user_${payload.sub}` : 'anonymous';
  } catch {
    return 'anonymous';
  }
};

const parserStorageKey = (key: string) => `eff_parser:${currentParserStorageScope()}:${key}`;

const readParserStorage = (key: string) => localStorage.getItem(parserStorageKey(key));

const writeParserStorage = (key: string, value: string) => localStorage.setItem(parserStorageKey(key), value);

const removeParserStorage = (key: string) => localStorage.removeItem(parserStorageKey(key));

const readStoredNumber = (key: string) => {
  const value = readParserStorage(key);
...
export default function LogParser() {
  const [rawText, setRawText] = useState(() => {
    const saved = readParserStorage('raw_text');
    return saved !== null ? saved : sample;
  });
  const [parsed, setParsed] = useState<Record<string, any>>(() => {
    const saved = readParserStorage('parsed');
    return saved ? JSON.parse(saved) : {};
  });
  const [isEditing, setIsEditing] = useState(false);
  const [matchedRules, setMatchedRules] = useState<unknown[]>(() => {
    const saved = readParserStorage('matched_rules');
    return saved ? JSON.parse(saved) : [];
  });
  const [formattedChat, setFormattedChat] = useState(() => readParserStorage('formatted_chat') || '');
  const [formattedExcel, setFormattedExcel] = useState(() => readParserStorage('formatted_excel') || '');
  const [ipAlerts, setIpAlerts] = useState<Array<{ message: string; list: string }>>(() => {
    const saved = readParserStorage('ip_alerts');
    return saved ? JSON.parse(saved) : [];
  });
  const [assetContext, setAssetContext] = useState<{ src_asset?: Record<string, any>; dst_asset?: Record<string, any> }>(() => {
    const saved = readParserStorage('asset_context');
    return saved ? JSON.parse(saved) : {};
  });
  const [deviceId, setDeviceId] = useState<number | undefined>(() => readStoredNumber('device_id'));
  const [messageTemplateId, setMessageTemplateId] = useState<number | undefined>(() => readStoredNumber('message_template_id'));
  const [excelTemplateId, setExcelTemplateId] = useState<number | undefined>(() => readStoredNumber('excel_template_id'));
  ...
  const isViewer = isOnlyViewer(currentUser);
  ...
  const webhookProvider = webhookCfg.provider;
  const webhookReady = webhookCfg.enabled !== false && (
    webhookProvider
      ? !!(webhookCfg?.[webhookProvider]?.url || webhookCfg?.url)
      : ['dingtalk', 'wecom', 'feishu'].some((key) => webhookCfg?.[key]?.enabled && webhookCfg?.[key]?.url)
  );
  ...
  const parse = useMutation({
    ...
    onSuccess: (data) => {
      ...
      writeParserStorage('parsed', JSON.stringify(data.parsed_fields || {}));
      writeParserStorage('matched_rules', JSON.stringify(data.matched_rules || []));
      writeParserStorage('formatted_chat', data.formatted_chat || '');
      writeParserStorage('formatted_excel', data.formatted_excel || '');
      writeParserStorage('ip_alerts', JSON.stringify(data.ip_list_alerts || []));
      writeParserStorage('asset_context', JSON.stringify(data.asset_context || {}));
  ...
  const updateField = (key: string, value: string) => {
    ...
    setParsed(next);
    writeParserStorage('parsed', JSON.stringify(next));
  ...
  const rememberSelection = (key: string, value?: number) => {
    if (value) {
      writeParserStorage(key, String(value));
    } else {
      removeParserStorage(key);
    }
  };
  const resetParser = () => {
    setRawText(sample);
    ...
    setIpAlerts([]);
    setAssetContext({});
    removeParserStorage('raw_text');
    removeParserStorage('parsed');
    removeParserStorage('matched_rules');
    removeParserStorage('formatted_chat');
    removeParserStorage('formatted_excel');
    removeParserStorage('ip_alerts');
    removeParserStorage('asset_context');
    message.info('解析器已重置');
  };
  ...
  }, [assetContext, formattedChat, formattedExcel, matchedRules, parsed, rawText, selectedDevice?.name]);
      ...
      <div className="page-toolbar">
        ...
        <Space wrap>
          ...
          <Select
            ...
            onChange={(value) => {
              setDeviceId(value);
              rememberSelection('device_id', value);
            }}
          ...
          <Select
            ...
            onChange={(value) => {
              ...
              rememberSelection('message_template_id', value);
            }}
          ...
          <Select
            ...
            onChange={(value) => {
              ...
              rememberSelection('excel_template_id', value);
            }}
        ...
        <Col span={10}>
          <Input.TextArea
            ...
            onChange={(e) => {
              ...
              setRawText(val);
              writeParserStorage('raw_text', val);
            }}
