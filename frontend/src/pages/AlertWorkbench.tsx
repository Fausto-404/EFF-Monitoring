      ...
      <Drawer open={!!selected} onClose={closeDrawer} width={760} title="告警详情">
        {selected && (
          <Space direction="vertical" size="large" className="full-width">
            ...
            <section>
              <Typography.Title level={5}>状态变更记录</Typography.Title>
              {history.length > 0 ? <ul className="history-list" style={{ paddingLeft: 20, margin: 0 }}>{history.map(renderHistoryItem)}</ul> : <Typography.Text type="secondary">暂无变更记录</Typography.Text>}
            </section>
            <section>
              <Typography.Title level={5}>资产上下文</Typography.Title>
            ...
            <section>
              <Typography.Title level={5}>AI 研判</Typography.Title>
              <pre>{selected.ai_result || '暂无'}</pre>
            </section>
            <section>
              <Typography.Title level={5}>原始日志</Typography.Title>
              <pre>{selected.raw_text}</pre>
            </section>
            <section>
              <Typography.Title level={5}>解析字段</Typography.Title>
              <pre>{JSON.stringify(selected.parsed_fields, null, 2)}</pre>
            </section>
          </Space>
        )}
      </Drawer>
