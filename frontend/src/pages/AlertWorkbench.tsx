...
import { Button, Card, Collapse, DatePicker, Descriptions, Drawer, Form, Input, Modal, Popconfirm, Radio, Select, Space, Switch, Table, Tag, Typography, message } from 'antd';
import dayjs from 'dayjs';
import type { Dayjs } from 'dayjs';
import { api } from '../api/client';
import type { Alert, Device, Project, Template, User } from '../api/types';
...
export default function AlertWorkbench({
  ...
  const { data: alertPage = { rows: [], total: 0 }, isLoading, isFetching, refetch } = useQuery({
    queryKey: ['alerts', q, status, currentGroup, projectId, assigneeId, alertStart, alertEnd, page, pageSize],
    queryFn: async () => {
  ...
  useEffect(() => {
    setPage(1);
  }, [q, status, currentGroup, projectId, assigneeId, alertStart, alertEnd]);
  ...
  const activeResponseUsers = users.filter((item) => item.role === 'disposer' && item.is_active);
        ...
        {selected && (
          <Space direction="vertical" size="large" className="full-width">
            <Descriptions bordered column={2} size="small">
              ...
              <Descriptions.Item label="是否应急">{selected.is_emergency ? '是' : '否'}</Descriptions.Item>
              <Descriptions.Item label="研判结果">{selected.analysis_result || '-'}</Descriptions.Item>
