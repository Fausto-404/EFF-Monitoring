...
export default function AlertWorkbench({
  ...
  const columns = useMemo(
    () => [
      { title: '告警ID', dataIndex: 'alert_code', width: 124, render: (v: string) => <Typography.Text className="table-nowrap">{v}</Typography.Text> },
      {
        title: '创建时间',
      ...
      { title: '源IP', dataIndex: 'source_ip', width: 120, render: (v: string) => <Typography.Text className="table-nowrap">{v || '-'}</Typography.Text> },
      { title: '目的IP', dataIndex: 'destination_ip', width: 120, render: (v: string) => <Typography.Text className="table-nowrap">{v || '-'}</Typography.Text> },
      { title: '事件类型', dataIndex: 'event_type', width: 160, render: (v: string) => <Typography.Text className="table-nowrap table-event-type" title={v || ''}>{v || '未识别'}</Typography.Text> },
      { title: '所属组', dataIndex: 'current_group', width: 84, render: (v: string) => <Typography.Text className="table-nowrap">{groupLabel[v] || v}</Typography.Text> },
      {
        title: '状态',
        dataIndex: 'status',
        width: 104,
      ...
      { title: '负责人', dataIndex: 'assignee_id', width: 92, render: (_: number | null, row: Alert) => <Typography.Text className="table-nowrap">{ownerDisplay(row, users)}</Typography.Text> },
      {
        title: '操作',
        width: 220,
