    ...
    <div className="page">
      ...
      <Modal title={editingTemplate ? '编辑模板' : '新增模板'} open={builderOpen} onCancel={() => setBuilderOpen(false)} onOk={saveBuilderTemplate} width={860}>
        <Form layout="vertical">
          <Form.Item label={<>模板名称<HelpTip title="模板名称会出现在日志解析的模板下拉框中，建议按场景命名，例如“封禁 IP 事件-消息模板”。" /></>}>
          ...
          <Form.Item label={<>适用设备<HelpTip title="不选择设备则所有设备可用；选择设备后，日志解析只有选择该设备时才会看到这个模板。" /></>}>
            <Select
              allowClear
