...
import { Alert, Button, Card, Col, DatePicker, Form, Input, Popconfirm, Row, Select, Space, Switch, Table, Tabs, Typography, message, Divider, Badge, AutoComplete, Tooltip } from 'antd';
import dayjs from 'dayjs';
import { api } from '../api/client';
import type { User } from '../api/types';
import HelpTip from '../components/HelpTip';
import { hasRole } from '../utils/roles';
import { Globe, User as UserIcon, Zap, Shield, Cloud, Database, Activity, CheckCircle2, XCircle, Sparkles, MessageSquare, Bot, RefreshCcw, Info } from 'lucide-react';
...
function webhookErrorText(detail: any, fallback: string) {
  if (!detail) return fallback;
  if (typeof detail === 'string') return detail;
  const details = detail.details || detail;
  const first = Object.values(details).find((item: any) => item?.error) as any;
...
function WebhookConfigForm({ initialValues, onSave, onTest, scope, isSaving, isTesting }: any) {
  ...
  useEffect(() => {
    form.resetFields();
    const provider = initialValues?.provider || (initialValues?.dingtalk?.url ? 'dingtalk' : initialValues?.wecom?.url ? 'wecom' : initialValues?.feishu?.url ? 'feishu' : 'dingtalk');
    const legacy = initialValues?.url ? { [provider]: { url: initialValues.url, secret: initialValues.secret } } : {};
    form.setFieldsValue({ enabled: true, provider, ...initialValues, ...legacy });
  }, [initialValues, scope, form]);

  const normalizeWebhook = (value: any) => {
    const provider = value.provider || 'dingtalk';
    return {
  ...
  return (
    <Card size="small" title="消息通知配置">
      <Form 
        form={form}
        layout="vertical" 
        onFinish={(value) => onSave(normalizeWebhook(value))}
      >
        <Form.Item name="enabled" label="启用 Webhook 通知" valuePropName="checked"><Switch /></Form.Item>
        <Form.Item name="provider" label="通知平台" rules={[{ required: true, message: '请选择通知平台' }]}>
          <Select options={[
        ...
        <Form.Item noStyle shouldUpdate={(prev, next) => prev.provider !== next.provider}>
          {({ getFieldValue }) => {
            const provider = getFieldValue('provider') || 'dingtalk';
            if (provider === 'dingtalk') {
              return (
                <>
                  <Typography.Text type="secondary" style={{ display: 'block', marginBottom: 12 }}>
                    钉钉机器人使用 access_token Webhook 地址；开启加签时填写安全设置中的 Secret。
                  </Typography.Text>
                  <Form.Item name={['dingtalk', 'url']} label="钉钉 Webhook URL" rules={[{ required: true, message: '请输入钉钉机器人 Webhook' }]}>
                    <Input placeholder="https://oapi.dingtalk.com/robot/send?access_token=..." />
                  </Form.Item>
                  <Form.Item name={['dingtalk', 'secret']} label="加签 Secret">
            ...
            if (provider === 'wecom') {
              return (
                <>
                  <Typography.Text type="secondary" style={{ display: 'block', marginBottom: 12 }}>
                    企业微信群机器人使用 key 型 Webhook，可选配置 @ 用户 ID 或手机号，多个值用逗号分隔。
                  </Typography.Text>
                  <Form.Item name={['wecom', 'url']} label="企业微信 Webhook URL" rules={[{ required: true, message: '请输入企业微信机器人 Webhook' }]}>
                    <Input placeholder="https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=..." />
                  </Form.Item>
                  <Row gutter={16}>
                    <Col span={12}>
                      <Form.Item name={['wecom', 'mentioned_list']} label="@成员 UserID">
                        <Input placeholder="@all 或 user1,user2" />
                      </Form.Item>
                    </Col>
                    <Col span={12}>
                      <Form.Item name={['wecom', 'mentioned_mobile_list']} label="@手机号">
            ...
            return (
              <>
                <Typography.Text type="secondary" style={{ display: 'block', marginBottom: 12 }}>
                  飞书机器人使用 webhook 地址；开启签名校验时填写机器人安全设置中的 Secret。
                </Typography.Text>
                <Form.Item name={['feishu', 'url']} label="飞书 Webhook URL" rules={[{ required: true, message: '请输入飞书机器人 Webhook' }]}>
                  <Input placeholder="https://open.feishu.cn/open-apis/bot/v2/hook/..." />
                </Form.Item>
                <Form.Item name={['feishu', 'secret']} label="签名 Secret">
                  <Input.Password placeholder="机器人安全设置中的签名密钥" />
                </Form.Item>
              </>
            );
          }}
...
function PluginAccessForm() {
  const [form] = Form.useForm();
  const [issuedToken, setIssuedToken] = useState('');
  const queryClient = useQueryClient();
  const platformUrl = typeof window !== 'undefined' ? window.location.origin : '';
  const scopes = ['plugin:devices:read', 'plugin:parse', 'plugin:alert:create', 'plugin:ai:investigate'];

  const { data: tokens = [], isLoading } = useQuery({
    queryKey: ['plugin-tokens'],
    queryFn: async () => (await api.get<PluginToken[]>('/api/plugin/tokens')).data,
  });

  const createToken = useMutation({
    mutationFn: async (values: any) => (await api.post('/api/plugin/tokens', {
      name: values.name || '浏览器助手密钥',
      scopes,
    ...
    onSuccess: (data) => {
      ...
      form.resetFields();
      queryClient.invalidateQueries({ queryKey: ['plugin-tokens'] });
      message.success('插件密钥已生成，请立即复制保存');
    },
    onError: (error: any) => message.error(error?.response?.data?.detail || '生成插件密钥失败')
  });

  const deleteToken = useMutation({
    mutationFn: async (id: number) => (await api.delete(`/api/plugin/tokens/${id}`)).data,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['plugin-tokens'] });
      message.success('插件密钥已删除');
    },
  ...
  return (
    <Space direction="vertical" className="full-width" size="large">
      <Card
        ...
        title="接入参数"
        extra={
          <Tooltip title="下载 EFF Assistant 浏览器插件。安装后可使用这里生成的 PAT 连接平台，同步设备配置、从安全设备页面生成工单，并调用平台证据增强研判。">
            <Button
              size="small"
              onClick={() => window.open('https://github.com/Fausto-404/EFF-Assistant', '_blank', 'noopener,noreferrer')}
            >
              下载插件
            </Button>
          </Tooltip>
      ...
      >
        <Space direction="vertical" className="full-width" size="middle">
          <div>
            <Typography.Text type="secondary">平台地址</Typography.Text>
            <Typography.Paragraph copyable={{ text: platformUrl }} style={{ marginBottom: 0 }}>
              <Typography.Text code>{platformUrl}</Typography.Text>
            </Typography.Paragraph>
          </div>
          <div>
            <Typography.Text type="secondary">默认权限</Typography.Text>
            <div style={{ marginTop: 8 }}>
              <Space wrap>{scopes.map((item) => <Badge key={item} color="#1677ff" text={<Typography.Text code>{item}</Typography.Text>} />)}</Space>
            </div>
          </div>
        </Space>
      </Card>
      <Card size="small" title="生成插件 PAT">
        <Form form={form} layout="vertical" onFinish={(values) => createToken.mutate(values)} initialValues={{ name: 'EFF Assistant 浏览器助手' }}>
          <Row gutter={12}>
            <Col span={12}>
              <Form.Item name="name" label="密钥名称"><Input /></Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item name="expires_at" label="过期时间">
                <DatePicker
                  ...
                  placeholder="可选，选择过期日期和时间"
                  disabledDate={(current) => !!current && current < dayjs().startOf('day')}
                />
              </Form.Item>
            </Col>
          </Row>
          <Button type="primary" htmlType="submit" loading={createToken.isPending}>生成 PAT</Button>
        </Form>
        {issuedToken && (
          <Alert
            style={{ marginTop: 12 }}
            ...
            showIcon
            message="密钥只展示一次"
            description={<Typography.Paragraph copyable={{ text: issuedToken }} style={{ marginBottom: 0 }}><Typography.Text code>{issuedToken}</Typography.Text></Typography.Paragraph>}
          />
        )}
      </Card>
      <Card size="small" title="已创建的插件密钥">
        <Table
          size="small"
          rowKey="id"
          loading={isLoading}
          ...
          columns={[
            ...
            { title: '前缀', dataIndex: 'token_prefix', width: 180, render: (v: string) => <Typography.Text code>{v}</Typography.Text> },
            { title: '最后使用', dataIndex: 'last_used_at', width: 170, render: (v: string | null) => v ? new Date(v).toLocaleString() : '-' },
            { title: '状态', dataIndex: 'revoked_at', width: 100, render: (v: string | null) => v ? <Badge status="default" text="已停用" /> : <Badge status="success" text="可用" /> },
            {
              title: '操作',
              width: 100,
              render: (_: unknown, row: PluginToken) => (
                <Popconfirm title="删除该插件密钥？" description="删除后插件将无法继续使用该密钥连接平台。" onConfirm={() => deleteToken.mutate(row.id)}>
                  <Button size="small" danger loading={deleteToken.isPending}>删除</Button>
                </Popconfirm>
              )
            }
          ]}
...
export default function SettingsPage() {
  ...
  const personalNoticeKey = 'eff_settings_personal_notice_hidden_v1';
  const [personalNoticeHidden, setPersonalNoticeHidden] = useState(() => (
    typeof window !== 'undefined' && window.localStorage.getItem(personalNoticeKey) === '1'
  ));
  const queryClient = useQueryClient();
  ...
  const webhookTest = useMutation({
    ...
    onError: (error: any) => message.error(webhookErrorText(error?.response?.data?.detail, '发送失败'))
  });
  ...
  const renderConfigForms = (scope: Scope) => {
    ...
    return (
      <Space direction="vertical" className="full-width" size="large">
        {isPersonal && !personalNoticeHidden && (
          <Alert
            closable
            onClose={() => setPersonalNoticeHidden(true)}
            action={
              <Button
                type="link"
                size="small"
                onClick={() => {
                  window.localStorage.setItem(personalNoticeKey, '1');
                  setPersonalNoticeHidden(true);
                }}
              >
                不再提醒
              </Button>
