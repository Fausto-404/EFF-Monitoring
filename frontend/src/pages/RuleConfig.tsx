  ...
  const content = (
    <>
      <div className="page-toolbar" style={isSubModule ? { padding: '0 0 16px 0', borderBottom: 'none' } : {}}>
        {!isSubModule && (
          <div>
            ...
            <Typography.Text type="secondary">配置日志解析正则表达式与字段提取规则</Typography.Text>
          </div>
        )}
      ...
      <Modal title={editing ? (editing.is_meta ? '配置元规则' : '编辑规则') : '新增规则'} open={open} onCancel={() => { setOpen(false); setEditing(null); }} onOk={() => form.submit()} width={720}>
        ...
        <Form form={form} layout="vertical" initialValues={{ match_type: 'regex', priority: 100, enabled: true }} onFinish={(values) => create.mutate(values)}>
          <Form.Item name="device_id" label={<>设备<HelpTip title="选择设备后，该规则只在日志解析选择同一设备时生效；不选择则为通用规则。" /></>}><Select allowClear options={devices.map((item) => ({ value: item.id, label: item.name }))} /></Form.Item>
          ...
          <Form.Item noStyle shouldUpdate={(prev, next) => prev.match_type !== next.match_type || prev.field_key !== next.field_key}>
            {({ getFieldValue }) => {
              ...
              return <Form.Item name="pattern" label={<>匹配规则<HelpTip title="正则必须包含捕获组，系统会优先取第一个捕获组作为字段值。跨行内容可使用 [\\s\\S]+?。" /></>} rules={[{ required: true }]}><Input.TextArea rows={4} /></Form.Item>;
            }}
          </Form.Item>
