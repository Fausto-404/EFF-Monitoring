...
+ 首次启动只初始化系统默认规则、内置模板和管理员账号
+ 不自动写入演示告警、演示资产、演示项目和演示用户
...
| 告警工作台 | Hash 搜索、认领机制、状态流转、AI 研判、威胁情报、闭环反馈、CSV 导出、安全事件跟踪表字段导出 |
...
| 系统管理 | 用户、项目、设备、设备类型（监测设备/封禁设备）、审计日志、后台任务、历史导入 |
...
## 可选演示数据
`v2.1.2` 打包目录默认关闭演示数据初始化，即 `.env` 中默认配置为 `ENABLE_DEMO_DATA=false`。
...
## 更新记录
### v2.1.2
+ **安全事件跟踪表导出增强**：系统内置新增 `安全事件跟踪表` CSV 模板，支持按固定字段顺序导出告警编号、告警时间、告警设备、攻击源/目的 IP、监测上报人员、研判人员、是否应急、研判结果、封禁位置、封禁人员、封禁时间、处置描述、应急人员等内容。
+ **处置流程字段补齐**：告警在研判和处置过程中新增 `监测上报人员`、`研判结果`、`是否应急`、`封禁位置`、`封禁时间`、`处置描述`、`应急人员` 等流程字段，导出结果可直接对齐安全事件跟踪表。
+ **封禁设备管理**：系统管理中的设备新增 `设备类型`，支持区分 `监测设备` 和 `封禁设备`。当研判结果为封禁类时，处置人员在流转到 `已处置` 前可多选对应封禁设备，导出时自动生成封禁位置。
+ **应急处置补充描述**：当研判结果为应急类时，处置人员在流转到 `已处置` 前可填写非必填的 `处置描述`，并可指定 `应急人员`，导出时一并带出。
+ **交付包默认无历史测试数据**：`v2.1.2` 版本目录默认不携带历史测试数据，不包含当前环境的 Docker 数据卷内容；首次启动仅保留系统默认规则、模板和管理员初始化。
